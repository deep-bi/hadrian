...
package data {
  ...
  object NumericalComparison {
    ...
    def apply(x: Float, y: Float): Int =
      if (java.lang.Float.isNaN(x)) {
        ...
        if (java.lang.Float.isNaN(y))
          -1
        else if (x > y) 1 else if (x < y) -1 else 0
      }
   ...
   *  - "record" are loaded into a dynamically generated subclass of [[com.opendatagroup.hadrian.data.PFARecord PFARecord]]
   *
   * @param pfaSpecificData data model that points to the custom `ClassLoader`
   */
  ...
  class PFAMap[X <: AnyRef](private val builder: mutable.Builder[(String, X), Map[String, X]], private var map: Map[String, X]) extends java.util.Map[String, X] {
     ...
     *
     * @param path    coordinates of the deep element to have replaced
     * @param updator new value for the subelement
     * @param schema  schema of the subelement
     * @return new PFAMap, leaving the old one untouched
     */
    ...
    def getTranslator(avroType: AvroType, classLoader: java.lang.ClassLoader, memo: mutable.Map[String, Translator]): Translator =
      memo.getOrElse(avroType.fullName, avroType match {
        ...
        case x: AvroUnion =>
          ...
          val translators =
            types collect {
              case y: TranslateArray => {
                case null => null;
                case datum: PFAArray[_] => y.translate(datum)
              }: PartialFunction[AnyRef, AnyRef]
              case y: TranslateMap => {
                case null => null;
                case datum: PFAMap[_] => y.translate(datum)
              }: PartialFunction[AnyRef, AnyRef]
              case y: TranslateFixed => {
                case null => null;
                case datum: AnyPFAFixed if (datum.getSchema.getFullName == y.fullName) => y.translate(datum)
              }: PartialFunction[AnyRef, AnyRef]
              case y: TranslateEnum => {
                case null => null;
                case datum: AnyPFAEnumSymbol if (datum.getSchema.getFullName == y.fullName) => y.translate(datum)
              }: PartialFunction[AnyRef, AnyRef]
              case y: TranslateRecord => {
                case null => null;
                case datum: AnyPFARecord if (datum.getSchema.getFullName == y.fullName) => y.translate(datum)
              }: PartialFunction[AnyRef, AnyRef]
            }
          new TranslateUnion((translators :+ ({
            case y => y
          }: PartialFunction[AnyRef, AnyRef])) reduce {
            _ orElse _
          })
      })
  }
    ...
    def getTranslator(avroType: AvroType, classLoader: java.lang.ClassLoader, memo: mutable.Map[String, Translator]): Translator =
      memo.getOrElse(avroType.fullName, avroType match {
        ...
        case x: AvroUnion =>
          ...
          val translators =
            types collect {
              case y: TranslateBytes => {
                case null => null;
                case datum: java.nio.ByteBuffer => y.translate(datum)
              }: PartialFunction[AnyRef, AnyRef]
              case y: TranslateString => {
                case null => null;
                case datum: java.lang.CharSequence => y.translate(datum)
              }: PartialFunction[AnyRef, AnyRef]
              case y: TranslateArray => {
                case null => null;
                case datum: java.util.List[_] => y.translate(datum)
              }: PartialFunction[AnyRef, AnyRef]
              case y: TranslateMap => {
                case null => null;
                case datum: java.util.Map[_, _] => y.translate(datum)
              }: PartialFunction[AnyRef, AnyRef]
              case y: TranslateFixed => {
                case null => null;
                case datum: org.apache.avro.generic.GenericFixed if (datum.getSchema.getFullName == y.fullName) => y.translate(datum)
              }: PartialFunction[AnyRef, AnyRef]
              case y: TranslateEnum => {
                case null => null;
                case datum: org.apache.avro.generic.GenericEnumSymbol if (datum.getSchema.getFullName == y.fullName) => y.translate(datum)
              }: PartialFunction[AnyRef, AnyRef]
              case y: TranslateRecord => {
                case null => null;
                case datum: org.apache.avro.generic.GenericRecord if (datum.getSchema.getFullName == y.fullName) => y.translate(datum)
              }: PartialFunction[AnyRef, AnyRef]
            }
  ...
  object ScalaDataTranslator {
    ...
    class TranslateRecord[X](avroRecord: AvroRecord, classLoader: java.lang.ClassLoader, memo: mutable.Map[String, Translator[_]]) extends Translator[X] {
      ...
      val constructor = classLoader.loadClass(fullName).getConstructor(fieldTranslators map {
        _.scalaClass
      }: _*)
      constructor.setAccessible(true)
      ...
      memo.getOrElse(avroType.fullName, avroType match {
        ...
        case x: AvroUnion =>
          ...
          val translateToScala = translators map {
            case y: TranslateNull => {
              case null => y.toScala(null)
            }: PartialFunction[AnyRef, Any]
            case y: TranslateBoolean => {
              case datum: java.lang.Boolean => y.toScala(datum)
            }: PartialFunction[AnyRef, Any]
            case y: TranslateInt => {
              case datum: java.lang.Integer => y.toScala(datum)
            }: PartialFunction[AnyRef, Any]
            case y: TranslateLong => {
              case datum: java.lang.Long => y.toScala(datum)
            }: PartialFunction[AnyRef, Any]
            case y: TranslateFloat => {
              case datum: java.lang.Float => y.toScala(datum)
            }: PartialFunction[AnyRef, Any]
            case y: TranslateDouble => {
              case datum: java.lang.Double => y.toScala(datum)
            }: PartialFunction[AnyRef, Any]
            case y: TranslateBytes => {
              case datum: Array[_] => y.toScala(datum)
            }: PartialFunction[AnyRef, Any]
            case y: TranslateString => {
              case datum: String => y.toScala(datum)
            }: PartialFunction[AnyRef, Any]
            case y: TranslateArray[_] => {
              case datum: PFAArray[_] => y.toScala(datum)
            }: PartialFunction[AnyRef, Any]
            case y: TranslateMap[_] => {
              case datum: PFAMap[_] => y.toScala(datum)
            }: PartialFunction[AnyRef, Any]
            case y: TranslateFixed => {
              case datum: AnyPFAFixed if (datum.getSchema.getFullName == y.fullName) => y.toScala(datum)
            }: PartialFunction[AnyRef, Any]
            case y: TranslateEnum => {
              case datum: AnyPFAEnumSymbol if (datum.getSchema.getFullName == y.fullName) => y.toScala(datum)
            }: PartialFunction[AnyRef, Any]
            case y: TranslateRecord[_] => {
              case datum: AnyPFARecord if (datum.getSchema.getFullName == y.fullName) => y.toScala(datum)
            }: PartialFunction[AnyRef, Any]
          }

          val translateFromScala = translators map {
            case y: TranslateNull => {
              case null => y.fromScala(null)
            }: PartialFunction[Any, AnyRef]
            case y: TranslateBoolean => {
              case datum: Boolean => y.fromScala(datum)
            }: PartialFunction[Any, AnyRef]
            case y: TranslateInt => {
              case datum: Int => y.fromScala(datum)
            }: PartialFunction[Any, AnyRef]
            case y: TranslateLong => {
              case datum: Long => y.fromScala(datum)
            }: PartialFunction[Any, AnyRef]
            case y: TranslateFloat => {
              case datum: Float => y.fromScala(datum)
            }: PartialFunction[Any, AnyRef]
            case y: TranslateDouble => {
              case datum: Double => y.fromScala(datum)
            }: PartialFunction[Any, AnyRef]
            case y: TranslateBytes => {
              case datum: Array[_] => y.fromScala(datum)
            }: PartialFunction[Any, AnyRef]
            case y: TranslateString => {
              case datum: String => y.fromScala(datum)
            }: PartialFunction[Any, AnyRef]
            case y: TranslateArray[_] => {
              case datum: Seq[_] => y.fromScala(datum)
            }: PartialFunction[Any, AnyRef]
            case y: TranslateMap[_] => {
              case datum: Map[_, _] => y.fromScala(datum)
            }: PartialFunction[Any, AnyRef]
            case y: TranslateFixed => {
              case datum: Array[_] if (y.size == datum.size) => y.fromScala(datum)
            }: PartialFunction[Any, AnyRef]
            case y: TranslateEnum => {
              case datum: String if (y.symbols.contains(datum)) => y.fromScala(datum)
            }: PartialFunction[Any, AnyRef]
            case y: TranslateRecord[_] => {
              case datum: AnyPFARecord if (datum.getSchema.getFullName == y.fullName) => y.fromScala(datum)
            }: PartialFunction[Any, AnyRef]
          }

          new TranslateUnion(translateToScala reduce {
            _ orElse _
          }, translateFromScala reduce {
            _ orElse _
          })
      })
  }
      ...
      fields map { field =>
        csvIndexLookup.get(field.name) match {
          ...
          case Some(i) => (i, field.avroType match {
            ...
            case AvroInt() => { x: String =>
              try {
                java.lang.Integer.valueOf(x.toInt)
              } catch {
                case _: java.lang.NumberFormatException => throw new IllegalArgumentException(s"""field ${field.name} declared int but found "$x"""")
              }
            }
            case AvroLong() => { x: String =>
              try {
                java.lang.Long.valueOf(x.toLong)
              } catch {
                case _: java.lang.NumberFormatException => throw new IllegalArgumentException(s"""field ${field.name} declared long but found "$x"""")
              }
            }
            case AvroFloat() => { x: String =>
              try {
                java.lang.Float.valueOf(x.toFloat)
              } catch {
                case _: java.lang.NumberFormatException => throw new IllegalArgumentException(s"""field ${field.name} declared float but found "$x"""")
              }
            }
            case AvroDouble() => { x: String =>
              try {
                java.lang.Double.valueOf(x.toDouble)
              } catch {
                case _: java.lang.NumberFormatException => throw new IllegalArgumentException(s"""field ${field.name} declared double but found "$x"""")
              }
            }
    ...
    val fieldConverters: Seq[(String, PartialFunction[AnyRef, String])] = outputType match {
      case AvroRecord(fields, _, _, _, _) => fields map { field =>
        (field.name, field.avroType match {
          case AvroNull() => {
            case x: AnyRef => "null"
          }: PartialFunction[AnyRef, String]
          case AvroBoolean() => {
            case java.lang.Boolean.TRUE => "true";
            case java.lang.Boolean.FALSE => "false"
          }: PartialFunction[AnyRef, String]
          case AvroInt() => {
            case x: java.lang.Number => x.intValue.toString
          }: PartialFunction[AnyRef, String]
          case AvroLong() => {
            case x: java.lang.Number => x.longValue.toString
          }: PartialFunction[AnyRef, String]
          case AvroFloat() => {
            case x: java.lang.Number => x.floatValue.toString
          }: PartialFunction[AnyRef, String]
          case AvroDouble() => {
            case x: java.lang.Number => x.doubleValue.toString
          }: PartialFunction[AnyRef, String]
          case AvroBytes() => {
            case x: Array[_] => new String(x.asInstanceOf[Array[Byte]])
          }: PartialFunction[AnyRef, String]
          case AvroString() => {
            case x: String => x
          }: PartialFunction[AnyRef, String]
